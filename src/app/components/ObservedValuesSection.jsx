...
const ObservedValuesSection = () => {
   ...
   return (
      <div className="bg-gray-300 border-2 border-gray-400 rounded h-full flex flex-col">
         ...
         <div className="p-3 space-y-2 pt-0 overflow-y-auto flex-1">
            {/* Date and Time Row */}
            ...
            <InputRow
               label="QNT (fmfm)"
               value={observedData.qnhFmFm}
               onChange={(v) => handleChange("qnhFmFm", v)}
               unit="Kts"
            />
            <InputRow
...
const InputRow = ({ label, value, onChange, unit }) => (
   <div className="flex items-center gap-2">
      <span className="text-xs font-bold w-40">{label}</span>
      <input
         type="text"
         value={value}
